...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    const cards = await Promise.all(languages.map((language) => loadTrustedTelegramEventCard(body.eventId as string, language)));
    const card = cards[languages.indexOf(body.language)];
