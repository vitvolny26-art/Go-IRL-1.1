...
const shareLanguages = ["ru", "uk", "cs", "en", "pl", "sk"] as const;
...
const versionPath = (eventId: string, alias: string, language: string) => `${eventId}/${alias}-${language}.version.txt`;
...
const existingPublicAlias = async (eventId: string) => { const result = await storageClient().storage.from(ACTIVITY_SHARE_CARD_BUCKET).list(eventId, { limit: 100 }); if (result.error) throw result.error; const jpeg = result.data.find((item) => item.name.endsWith(".jpg") && isActivitySharePublicAlias(item.name.slice(0, -4))); if (jpeg) return jpeg.name.slice(0, -4); for (const item of result.data) { const match = item.name.match(/^([A-Z][a-z]{2}[0-9]{6}_[a-z]{1,2})-(?:ru|uk|cs|en|pl|sk)\.jpg$/); if (match && isActivitySharePublicAlias(match[1])) return match[1]; } return null; };
