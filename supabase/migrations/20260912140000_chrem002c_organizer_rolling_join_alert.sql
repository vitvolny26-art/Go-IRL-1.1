...
begin
  ...
  if position('activity.organizer_join_alert' in v_definition) = 0 then
    v_definition := regexp_replace(
      v_definition,
      $claim_kind_lists$(notification\.kind (not )?in \(\s*'post_event\.organizer_confirmation',\s*'post_event\.participant_confirmation')(\s*\))$claim_kind_lists$,
      E'\\1,\n      ''activity.organizer_join_alert''\\3',
      'g'
    );
    if position('activity.organizer_join_alert' in v_definition) = 0 then
      raise exception 'chrem002c_notification_claim_kind_lists_shape_changed';
    end if;
  end if;
