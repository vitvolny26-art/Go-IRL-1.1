...
alter table public.cinema_movies
  add column if not exists director text;

alter table public.cinema_movies
  add column if not exists lead_actors jsonb not null default '[]'::jsonb;

alter table public.cinema_movies
  drop constraint if exists cinema_movies_lead_actors_array_check;

alter table public.cinema_movies
  add constraint cinema_movies_lead_actors_array_check
  check (jsonb_typeof(lead_actors) = 'array');
...
comment on column public.cinema_movies.director is
  'Primary credited director from verified movie metadata enrichment; null when unavailable.';
comment on column public.cinema_movies.lead_actors is
  'Ordered JSON array of principal credited actor names from verified movie metadata enrichment.';
comment on function public.city_posters_cinema_catalog(text, integer) is
  'Public City Posters cinema projection including verified director and principal actor metadata when available. Weekly publication selection semantics are unchanged.';
