-- GROOMING008 corrective canonicalization.
-- Repository-only migration. Production apply requires separate explicit approval.
-- Purpose: converge the two historical reminder implementations to one canonical trigger/function.

begin;

-- Remove both historical trigger/function variants if either was applied in any environment.
drop trigger if exists beauty_booking_events_queue_reminders on public.beauty_booking_events;
drop trigger if exists go_irl_sync_beauty_booking_reminders on public.beauty_booking_events;
drop function if exists public.go_irl_queue_beauty_booking_reminders();
drop function if exists public.go_irl_sync_beauty_booking_reminders();
...
begin
  ...
  -- Cancel reminders that have not yet been leased when the canonical booking state/time changes.
  ...
  if not found or v_booking.client_user_key is null then
    return new;
  end if;
  ...
  loop
    ...
    -- Do not enqueue a late reminder after its intended due time.
...
-- Backfill future reminders for currently confirmed bookings. Idempotency is enforced by delivery_key.
insert into public.event_notifications (
  user_key,
  activity_id,
  kind,
