...
const labels: Record<Language, {
  ...
  ru: { title: "Кино", subtitle: "Фильмы и ближайшие показы", showing: "В кино", schedule: "Показы", about: "О фильме", empty: "Сейчас нет актуальных показов.", error: "Не удалось загрузить расписание.", min: "мин" },
...
const render = () => {
  ...
  if (!loading && (loadError || !rows.length)) {
    surface.append(element("div", "cinema-feed-section__empty", loadError ? copy.error : copy.empty));
    return;
  }
