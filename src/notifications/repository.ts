...
export class EventNotificationRepository {
  ...
  async claim(limit = 50): Promise<EventNotificationDelivery[]> {
    const { data, error } = await withTransientSupabaseRpcRetry(() => this.client.rpc("go_irl_claim_event_notifications", { p_providers: this.providers, p_limit: limit, p_lease_seconds: 300 }));
