...
describe("Akce001B Telegram City Posters publication",()=>{
 ...
 it("formats prepared all-day shares as a date range without midnight time",()=>{expect(prepared).toContain('starts_at,ends_at,timezone,occurrence_url,metadata');expect(prepared).toContain("occurrence.metadata?.allDay === true");expect(prepared).toContain("endsAt.getTime() - 1");expect(prepared).toContain("day: \"numeric\", month: \"long\", timeZone");expect(prepared).toContain("const caption = [card.title, card.description, card.date, card.venue]");});
 ...
 it("defines a protected five-minute scheduler using the existing worker secret",()=>{expect(scheduler).toContain("go-irl-city-posters-maintenance");expect(scheduler).toContain("'*/5 * * * *'");expect(scheduler).toContain("go_irl_reminder_worker_secret");expect(scheduler).toContain("maintain_city_poster_publications");expect(scheduler).toContain("revoke all on function public.go_irl_configure_city_posters_maintenance_schedule()")});
});
