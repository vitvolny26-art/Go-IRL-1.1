...
const barberCopy: Record<Language, Partial<Record<keyof (typeof copy)["en"], string>>> = {
  ...
  pl: { title: "Barber workspace and services", portfolio: "Work", prices: "Services", publicName: "Barber name", publicLocation: "Barbershop / area", contact: "Barber contact", description: "About the barber", pricesHint: "Add barber services with a name, duration, and price.", addService: "Add barber service" },
  sk: { title: "Barber kabinet a služby", portfolio: "Práce", prices: "Služby", publicName: "Jméno barbera", publicLocation: "Barbershop / oblast", contact: "Kontakt barbera", description: "O barberovi", pricesHint: "Přidejte barber služby: název, délku a cenu.", addService: "Přidat barber službu" },
};
...
const professionalPhotoCopy: Record<Language, { title: string; hint: string; upload: string; remove: string }> = {
  ...
  pl: { title: "Professional photo", hint: "Shown on the public profile and stored in the existing portfolio JSON contract.", upload: "Upload professional photo", remove: "Remove photo" },
  sk: { title: "Fotografie profesionála", hint: "Zobrazuje se ve veřejném profilu a ukládá se do stávajícího portfolio JSON.", upload: "Nahrát fotografii", remove: "Odstranit fotografii" },
};
...
const translationCopy: Record<Language, { copyPrompt: string; copied: string; copyError: string; importAll: string; importHint: string; paste: string; apply: string; cancel: string; applied: string; importError: string }> = {
  ...
  pl: { copyPrompt: "Copy text for AI", copied: "AI prompt copied", copyError: "Could not copy the AI prompt", importAll: "Load all languages", importHint: "Paste the AI JSON response. RU / UK / CS / EN / PL / SK will be loaded atomically; the source language will stay unchanged.", paste: "AI response as JSON", apply: "Apply translations", cancel: "Cancel", applied: "Translations loaded", importError: "The AI response does not match the required structure. Check the JSON and all languages." },
  sk: { copyPrompt: "Zkopírovat text pro AI", copied: "Prompt pro AI byl zkopírován", copyError: "Prompt se nepodařilo zkopírovat", importAll: "Nahrát všechny jazyky", importHint: "Vložte JSON odpověď AI. RU / UK / CS / EN / PL / SK se nahrají najednou; zdrojový jazyk zůstane beze změny.", paste: "Odpověď AI v JSON", apply: "Použít překlady", cancel: "Zrušit", applied: "Překlady byly nahrány", importError: "Odpověď AI neodpovídá struktuře. Zkontrolujte JSON a všechny jazyky." },
};
...
export function BeautyWorkspaceContentEditor({ workspace, language, onChange }: { workspace: BeautyWorkspace; language: Language; onChange: (next: BeautyWorkspace) => void }) {
  ...
  return <section className="beauty-workspace-content-editor">
    ...
    {tab === "prices" && <div className="beauty-workspace-price-editor">
      <p>{text.pricesHint}</p>
      <datalist id={`beauty-service-presets-${professionId}-${contentLanguage}`}>{serviceSuggestions.map((name) => <option key={name} value={name} />)}</datalist>
