...
export type NotificationSubjectType = "activity" | "team" | "chat" | "message" | "review" | "profile" | "beauty_booking" | "weather_alert" | "system";
...
export type NotificationPreference = { userKey: string; kind: NotificationKind; inAppEnabled: boolean; channels: NotificationChannel[]; mutedUntil?: string | null; updatedAt: string };
export type NotificationRegistryEntry = { kind: NotificationKind; category: NotificationCategory; serviceCritical: boolean; defaultChannels: readonly NotificationChannel[]; retentionDays: number };
...
export const notificationRegistryByKind = new Map<NotificationKind, NotificationRegistryEntry>(notificationRegistry.map((entry) => [entry.kind, entry]));
export const getNotificationRegistryEntry = (kind: NotificationKind) => { const entry = notificationRegistryByKind.get(kind); if (!entry) throw new Error(`unknown_notification_kind:${kind}`); return entry };
export const buildNotificationDeduplicationKey = (recipientUserKey: string, kind: NotificationKind, subject: NotificationSubject, occurrenceKey: string) => [recipientUserKey, kind, subject.type, subject.id, occurrenceKey].map(encodeURIComponent).join(":");
export const canDisableNotification = (kind: NotificationKind) => !getNotificationRegistryEntry(kind).serviceCritical;
